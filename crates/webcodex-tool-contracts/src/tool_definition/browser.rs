...
use crate::registry::input_schemas::{browser_act_input_schema, browser_observe_input_schema};
...
mod tests {
    ...
    fn browser_model_surface_metrics_are_bounded() {
        ...
        let observe_schema_bytes = serde_json::to_vec(&(observe.input_schema)()).unwrap().len();
        let act_schema_bytes = serde_json::to_vec(&(act.input_schema)()).unwrap().len();
